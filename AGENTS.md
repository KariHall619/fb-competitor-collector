...
- `enrichment_worker.py` must mark `engagement` and `post_type` tasks done only when the same field-audit reasons used for Feishu `待补抓：...` have cleared. Low likes, missing comments/shares, or unsupported post type should keep the task open/failed for refetch instead of inflating final usable rate.
