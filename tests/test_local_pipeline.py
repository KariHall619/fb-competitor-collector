...
def assert_dom_extractor_excludes_comment_only_permalink_candidate() -> None:
    script = """
const { browserExpression } = require('./scripts/fb_dom_extractors');
class Node {
  constructor(tagName, attrs = {}, children = [], ownText = '') {
...
def assert_dom_extractor_keeps_photo_chrome_as_media_fallback_only() -> None:
    script = """
const { browserExpression } = require('./scripts/fb_dom_extractors');
class Node {
  constructor(tagName, attrs = {}, children = [], ownText = '') {
...
const chrome = new Node('div', { role: 'article' }, [
  new Node('span', {}, [], 'Facebook'),
  new Node('span', {}, [], 'Facebook'),
  new Node('a', { href: '/photo/?fbid=1596682352022768&set=a.863890281968649&__cft__[0]=abc' }, [], 'Photo'),
  new Node('span', {}, [], 'Healing Journeys'),
  new Node('p', {}, [], 'A Millionaire Stopped for a Girl With a Red Backpack and found a secret.'),
  new Node('a', { href: 'https://windowarab.com/a-millionaire-stopped-for-a-girl/' }, [], 'Read more')
]);
const body = new Node('body', {}, [
  new Node('h1', {}, [], 'Healing Journeys'),
  chrome
...
global.location = new URL('https://www.facebook.com/healingjourneys.mani');
const result = eval(browserExpression(1000));
if (result.real_post_count !== 1 || result.candidates.length !== 1) {
  console.error(JSON.stringify(result, null, 2));
  process.exit(1);
}
const candidate = result.candidates[0];
if (candidate.source_split !== 'media_fallback' || candidate.post_time_text || candidate.lead_link_status === 'qualified') {
  console.error(JSON.stringify(candidate, null, 2));
  process.exit(2);
}
"""
...
def assert_detail_enrichment_rejects_media_resource_comment_links() -> None:
    script = """
import detailExtractors from './scripts/fb_detail_extractors.js';
const { leadLinkScanBrowserExpression } = detailExtractors;
...
class Node {
  ...
  get textContent() { return this.innerText; }
  get href() { return this.attrs.href ? new URL(this.attrs.href, global.location.href).href : ''; }
  getAttribute(name) { return this.attrs[name] || ''; }
  closest(selector) {
    let node = this;
    while (node) {
...
const mediaOnly = new Node('div', { role: 'article' }, [
  new Node('span', {}, [], 'Healing Journeys'),
  new Node('span', {}, [], 'Sticker'),
  new Node('a', { href: 'https://media2.giphy.com/media/v1/test/giphy.gif' }, [], 'GIPHY'),
  new Node('a', { href: '/healingjourneys.mani/posts/pfbid?comment_id=123' }, [], '48m'),
  new Node('span', {}, [], 'Reply')
]);
const body = new Node('body', {}, [mediaOnly]);
global.document = { querySelectorAll: (selector) => body.querySelectorAll(selector) };
global.location = new URL('https://www.facebook.com/healingjourneys.mani/posts/pfbid');
const results = eval(leadLinkScanBrowserExpression('Healing Journeys', 'default'));
if (results.length !== 0) {
  console.error(JSON.stringify(results, null, 2));
  process.exit(1);
}
...
def assert_opencli_extract_helpers_dedupe_homepage_candidates() -> None:
    ...
    current_tab_text = (ROOT / "scripts" / "opencli_extract_current_tab.mjs").read_text(encoding="utf-8")
    assert 'candidate.source_split === "media_fallback"' in current_tab_text
    assert 'candidate.selected_post_link_kind === "media"' in current_tab_text
...
def assert_detail_time_target_ignores_comment_permalink_time() -> None:
    script = """
const { headerTimeTargetExpression } = require('./scripts/fb_detail_extractors.js');
...
class Node {
  constructor(tagName, attrs = {}, children = [], ownText = '', rect = {}) {
    ...
    this.children = children;
    this.ownText = ownText;
    this.rect = { x: 0, y: 120, width: 22, height: 14, ...rect };
    this.parentElement = null;
    for (const child of children) child.parentElement = this;
  }
  get innerText() {
...
const targetUrl = 'https://www.facebook.com/healingjourneys.mani/posts/pfbid-main';
const commentOnly = new Node('div', { role: 'article' }, [
  new Node('span', {}, [], 'Author'),
  new Node('strong', {}, [], 'Healing Journeys'),
  new Node('a', { href: '/healingjourneys.mani/posts/pfbid-main?comment_id=123&reply_comment_id=456' }, [], '4h', { y: 180, width: 20 }),
  new Node('a', { href: 'https://windowarab.com/story/' }, [], 'Part 3'),
  new Node('span', {}, [], 'Like'),
  new Node('span', {}, [], 'Reply'),
]);
const body = new Node('body', {}, [commentOnly]);
global.location = new URL(targetUrl);
global.window = { innerHeight: 800 };
global.document = {
  body,
...
def assert_detail_time_force_rechecks_confirmed_times() -> None:
    adapter_text = (ROOT / "scripts" / "opencli_fb_competitor_posts.js").read_text(encoding="utf-8")
    worker_text = (ROOT / "scripts" / "enrichment_worker.py").read_text(encoding="utf-8")
    extract_start = adapter_text.index("async function extractExactTime")
    extract_end = adapter_text.index("async function readRealMouseTooltipTime")
    extract_slice = adapter_text[extract_start:extract_end]
    assert "!options.forceTime" in extract_slice
    assert "{ name: 'force-time'" in adapter_text
    assert "forceTime: boolArg(kwargs['force-time'])" in adapter_text
    assert '"--force-time"' in worker_text


def assert_detail_navigation_preserves_photo_query_fbid() -> None:
    adapter_text = (ROOT / "scripts" / "opencli_fb_competitor_posts.js").read_text(encoding="utf-8")
    detail_start = adapter_text.index("function detailNavigationUrl")
    detail_end = adapter_text.index("function validCandidate", detail_start)
    detail_slice = adapter_text[detail_start:detail_end]
    assert "photo/?fbid=" in detail_slice
    assert "encodeURIComponent(fbid)" in detail_slice
    assert "photo.php" in detail_slice
...
def assert_enrichment_worker_lead_stage_requires_external_landing_url() -> None:
    ...
    media_lead = {
        "lead_link_status": "qualified",
        "lead_link_source": "comment",
        "lead_url_raw": "https://media2.giphy.com/media/v1/test/giphy.gif",
        "landing_url": "https://media2.giphy.com/media/v1/test/giphy.gif",
        "article_url": "https://media2.giphy.com/media/v1/test/giphy.gif",
    }
    assert enrichment_worker.detail_stage_satisfied(internal_lead, "lead_link") is False
    assert enrichment_worker.detail_stage_satisfied(media_lead, "lead_link") is False
...
def assert_run_accounts_job_follows_unsynced_recovery_when_sync_requested(tmp_path: Path) -> None:
    ...
    fake_python = tmp_path / "fake-python-unsynced-recovery"
    calls_file = tmp_path / "batch-unsynced-recovery-calls.json"
    shutil.copy(ROOT / "config" / "settings.yaml.example", config)
    text = config.read_text(encoding="utf-8")
    text = text.replace("lark_cli_path: auto", f"lark_cli_path: {fake_lark}")
    text = text.replace("database_path: data/posts.sqlite", f"database_path: {tmp_path / 'batch_unsynced_recovery.sqlite'}")
    text = text.replace('source_spreadsheet_url: ""', 'source_spreadsheet_url: "https://fake.feishu.cn/sheets/source"')
    config.write_text(text, encoding="utf-8")
    fake_lark.write_text(
        """#!/usr/bin/env python3
...
payload = {
  "data": {
    "valueRange": {
      "values": [
        ["主页名称", "竞品fb账户", "内部FB账户"],
        ["Unsynced Batch Page", "https://www.facebook.com/unsyncedbatch", ""]
      ]
    }
  }
}
...
config_path = sys.argv[sys.argv.index("--config") + 1]
account_calls = [call for call in calls if "--account-url" in call and call[call.index("--account-url") + 1] == account_url]
sync_command = "python3 scripts/run_account_job.py --config " + config_path + " --account-url " + account_url + " --account-name '" + account_name + "' --account-type " + account_type + " --target-date 260603 --resume-only --force-recover-running --sync --dry-run --fail-on-incomplete --max-resume-passes 8"
if len(account_calls) == 1:
    payload = {{
        "ok": True,
        "run_status": "captured_not_synced",
        "complete": False,
        "account_url": account_url,
        "account_name": account_name,
        "account_type": account_type,
        ...
        "next_commands": [{{
            "reason": "captured_not_synced",
            "description": "sync completed local rows",
...
""",
    ...
    assert data["run_status"] == "complete"
    account = data["accounts"][0]
    assert [attempt["run_status"] for attempt in account["attempts"]] == ["captured_not_synced", "complete"]
...
def assert_run_accounts_job_child_opencli_blocker_preserves_batch_retry(tmp_path: Path) -> None:
    config = tmp_path / "settings_batch_child_opencli_blocker.yaml"
    fake_lark = tmp_path / "fake-lark-cli-child-opencli-blocker"
    fake_python = tmp_path / "fake-python-child-opencli-blocker"
    calls_file = tmp_path / "batch-child-opencli-blocker-calls.json"
    shutil.copy(ROOT / "config" / "settings.yaml.example", config)
    text = config.read_text(encoding="utf-8")
    text = text.replace("lark_cli_path: auto", f"lark_cli_path: {fake_lark}")
    text = text.replace("database_path: data/posts.sqlite", f"database_path: {tmp_path / 'batch_child_opencli_blocker.sqlite'}")
    text = text.replace('source_spreadsheet_url: ""', 'source_spreadsheet_url: "https://fake.feishu.cn/sheets/source"')
    config.write_text(text, encoding="utf-8")
    fake_lark.write_text(
        """#!/usr/bin/env python3
...
payload = {
  "data": {
    "valueRange": {
      "values": [
        ["主页名称", "竞品fb账户", "内部FB账户"],
        ["Detail Blocked Page", "https://www.facebook.com/detailblocked", ""],
        ["Good Page", "https://www.facebook.com/goodchildpage", ""]
      ]
    }
  }
}
...
account_type = sys.argv[sys.argv.index("--account-type") + 1]
config_path = sys.argv[sys.argv.index("--config") + 1]
if "detailblocked" in account_url:
    ...
    payload = {{
        ...
        "quality_summary": {{
            ...
            "missing_stage_counts": {{"post_type": 2}},
            "top_field_gaps": [{{"reason": "post_type", "stage": "post_type", "count": 2}}]
        }},
        "next_commands": [
            {{
                "reason": "blocked_opencli",
                "description": "fix OpenCLI",
                "command": "python3 scripts/check_env.py --config " + config_path + " --fix-opencli"
            }},
            {{
                "reason": "resume_after_opencli",
                "description": "resume one account",
                "command": resume_command
            }}
        ]
    }}
...
""",
    ...
    assert data["accounts_completed"] == 1
    assert data["accounts_hard_blocked"] == 1
    assert [item["reason"] for item in data["next_commands"][:3]] == [
        "blocked_opencli",
    ...
    assert "--require-coverage-complete" in batch_rerun
    assert batch_rerun[batch_rerun.index("--min-final-usable-rate") + 1] == "0.9"
    single_resume = shlex.split(data["next_commands"][2]["command"])
    assert "scripts/run_account_job.py" in single_resume
    assert single_resume[single_resume.index("--account-url") + 1] == "https://www.facebook.com/detailblocked"
    assert "--resume-only" in single_resume
    assert "--force-recover-running" in single_resume


def assert_run_accounts_job_child_auth_blocker_preserves_batch_retry(tmp_path: Path) -> None:
    config = tmp_path / "settings_batch_child_auth_blocker.yaml"
    fake_lark = tmp_path / "fake-lark-cli-child-auth-blocker"
    fake_python = tmp_path / "fake-python-child-auth-blocker"
    calls_file = tmp_path / "batch-child-auth-blocker-calls.json"
    shutil.copy(ROOT / "config" / "settings.yaml.example", config)
    text = config.read_text(encoding="utf-8")
    text = text.replace("lark_cli_path: auto", f"lark_cli_path: {fake_lark}")
    text = text.replace("database_path: data/posts.sqlite", f"database_path: {tmp_path / 'batch_child_auth_blocker.sqlite'}")
    text = text.replace('source_spreadsheet_url: ""', 'source_spreadsheet_url: "https://fake.feishu.cn/sheets/source"')
    config.write_text(text, encoding="utf-8")
    fake_lark.write_text(
        """#!/usr/bin/env python3
...
payload = {
  "data": {
    "valueRange": {
      "values": [
        ["主页名称", "竞品fb账户", "内部FB账户"],
        ["Auth Blocked Page", "https://www.facebook.com/authchildblocked", ""],
        ["Good Page", "https://www.facebook.com/goodauthchild", ""]
      ]
    }
  }
}
...
account_type = sys.argv[sys.argv.index("--account-type") + 1]
config_path = sys.argv[sys.argv.index("--config") + 1]
if "authchildblocked" in account_url:
    resume_command = "python3 scripts/run_account_job.py --config " + config_path + " --account-url " + account_url + " --account-name '" + account_name + "' --account-type " + account_type + " --target-date 260603 --resume-only --force-recover-running --sync --fail-on-incomplete --max-resume-passes 9 --enrichment-limit 25"
    payload = {{
        "ok": False,
        "run_status": "blocked_auth",
        "complete": False,
        ...
        "quality_summary": {{
            ...
            "missing_stage_counts": {{"post_type": 2}},
            "top_field_gaps": [{{"reason": "post_type", "stage": "post_type", "count": 2}}],
            "feishu_sync": {{"enabled": True, "ok": False, "run_status": "blocked_auth"}}
        }},
        "next_commands": [
            {{
                "reason": "blocked_auth",
                "description": "restore Feishu auth",
                "command": "python3 scripts/check_env.py --config " + config_path + " --fix-auth"
            }},
            {{
                "reason": "blocked_auth",
                "description": "resume one account after auth",
                "command": resume_command
            }}
        ]
    }}
...
""",
    ...
    assert [item["reason"] for item in data["next_commands"][:3]] == [
        "blocked_auth",
        "rerun_batch_after_auth",
        "blocked_auth",
    ]
    assert "--fix-auth" in data["next_commands"][0]["command"]
    batch_rerun = shlex.split(data["next_commands"][1]["command"])
    assert "scripts/run_accounts_job.py" in batch_rerun
    assert batch_rerun[batch_rerun.index("--config") + 1] == str(config)
    assert batch_rerun[batch_rerun.index("--target-date") + 1] == "260603"
    ...
    single_resume = shlex.split(data["next_commands"][2]["command"])
    assert "scripts/run_account_job.py" in single_resume
    assert single_resume[single_resume.index("--account-url") + 1] == "https://www.facebook.com/authchildblocked"
    assert "--resume-only" in single_resume
    assert "--force-recover-running" in single_resume
...
def assert_run_accounts_job_worker_failed_preserves_batch_retry(tmp_path: Path) -> None:
    config = tmp_path / "settings_batch_worker_failed.yaml"
    fake_lark = tmp_path / "fake-lark-cli-worker-failed"
    fake_python = tmp_path / "fake-python-worker-failed"
    calls_file = tmp_path / "batch-worker-failed-calls.json"
    shutil.copy(ROOT / "config" / "settings.yaml.example", config)
    text = config.read_text(encoding="utf-8")
    text = text.replace("lark_cli_path: auto", f"lark_cli_path: {fake_lark}")
    text = text.replace("database_path: data/posts.sqlite", f"database_path: {tmp_path / 'batch_worker_failed.sqlite'}")
    text = text.replace('source_spreadsheet_url: ""', 'source_spreadsheet_url: "https://fake.feishu.cn/sheets/source"')
    config.write_text(text, encoding="utf-8")
    fake_lark.write_text(
        """#!/usr/bin/env python3
...
payload = {
  "data": {
    "valueRange": {
      "values": [
        ["主页名称", "竞品fb账户", "内部FB账户"],
        ["Worker Broken Page", "https://www.facebook.com/workerbrokenchild", ""],
        ["Good Page", "https://www.facebook.com/goodworkerchild", ""]
      ]
    }
  }
}
...
account_type = sys.argv[sys.argv.index("--account-type") + 1]
config_path = sys.argv[sys.argv.index("--config") + 1]
if "workerbrokenchild" in account_url:
    resume_command = "python3 scripts/run_account_job.py --config " + config_path + " --account-url " + account_url + " --account-name '" + account_name + "' --account-type " + account_type + " --target-date 260603 --resume-only --force-recover-running --sync --dry-run --fail-on-incomplete --max-resume-passes 9 --enrichment-limit 25"
    payload = {{
        "ok": False,
        "run_status": "worker_failed",
        "complete": False,
        "account_url": account_url,
        "account_name": account_name,
        "account_type": account_type,
        ...
        "next_commands": [
            {{
                ...
                "description": "resume one account after worker fix",
                "command": resume_command
            }}
        ]
    }}
...
""",
    ...
    single_resume = shlex.split(data["next_commands"][1]["command"])
    assert "scripts/run_account_job.py" in single_resume
    assert single_resume[single_resume.index("--account-url") + 1] == "https://www.facebook.com/workerbrokenchild"
    assert "--resume-only" in single_resume
    assert "--force-recover-running" in single_resume
...
def assert_run_accounts_job_accounts_load_failed_preserves_batch_retry(tmp_path: Path) -> None:
    config = tmp_path / "settings_batch_accounts_load_failed.yaml"
    fake_lark = tmp_path / "fake-lark-cli-accounts-load-failed"
    shutil.copy(ROOT / "config" / "settings.yaml.example", config)
    text = config.read_text(encoding="utf-8")
    text = text.replace("lark_cli_path: auto", f"lark_cli_path: {fake_lark}")
    text = text.replace("database_path: data/posts.sqlite", f"database_path: {tmp_path / 'batch_accounts_load_failed.sqlite'}")
    text = text.replace('source_spreadsheet_url: ""', 'source_spreadsheet_url: "https://fake.feishu.cn/sheets/source"')
    config.write_text(text, encoding="utf-8")
    fake_lark.write_text(
        """#!/bin/sh
echo "source workbook unavailable" >&2
exit 1
""",
        encoding="utf-8",
    )
    ...
    assert result.returncode == 1, result.stdout or result.stderr
    data = json.loads(result.stdout)
    assert data["run_status"] == "accounts_load_failed"
    assert data["complete"] is False
    assert data["next_commands"][0]["reason"] == "accounts_load_failed"
...
def main() -> int:
    ...
    assert_detail_enrichment_rejects_media_resource_comment_links()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert_detail_time_target_ignores_comment_permalink_time()
        ...
        assert_detail_time_force_rechecks_confirmed_times()
        assert_detail_navigation_preserves_photo_query_fbid()
