...
- `run_account_job.py --resume-only` recovers scoped stale `running` enrichment tasks before worker passes. The default is intentionally conservative at 30 minutes; use `--resume-stale-running-seconds` only when an operator knows the prior process was interrupted. Fresh active `running` tasks must stay locked to avoid duplicate detail navigation.
