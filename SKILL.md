...
- `run_account_job.py --resume-only` recovers scoped stale `running` enrichment tasks before worker passes. The default stale window is 30 minutes; only shorten it with `--resume-stale-running-seconds` when the previous process is known to be dead.
