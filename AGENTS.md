...
- `run_account_job.py --resume-only` recovers scoped stale `running` enrichment tasks before worker passes. The default is intentionally short so Codex/terminal interruptions can resume quickly; active `running` tasks are preserved to avoid duplicate detail navigation.
