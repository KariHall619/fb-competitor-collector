...
- “采集全部目标账号/所有账号今天的帖子并写入飞书”
  ...
  - It reads the Feishu account source sheet, calls the full `run_account_job.py` flow for each enabled account, automatically follows same-account machine-runnable recovery commands for the base `--auto-follow-attempts` budget (default 8), extends while field-completion quality is still improving, and aggregates per-account `run_status`, `completion_blockers`, and `next_commands`.
