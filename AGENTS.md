...
- Homepage capture should avoid stopping on a few stable DOM snapshots while Facebook can still scroll. `opencli_extract_current_tab.mjs` uses a higher default snapshot budget, a minimum snapshot count, and scroll-movement guards; stable no-new-post termination is normal completion, while `coverage_incomplete=true` means the last allowed snapshot still found new candidates and the operator should raise `--max-snapshots` or retry from the page top.
