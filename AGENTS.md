...
- The user-validated workflow is:
  1. Interpret the user request scope first: all configured accounts, one account, a direct account URL not yet in the Feishu source sheet, a specific post URL, or existing local/JSON/CSV/SQLite rows. Ask only when scope cannot be inferred from the request plus config.
  2. Before live capture, import with real sync, deployment/share, or Feishu write, run configuration/dependency preflight and verify required env/config, Feishu source/output settings, lark-cli user auth, OpenCLI daemon, and Browser Bridge profile connection.
  3. Codex should silently fix recoverable setup problems and re-run the check: lark-cli user-mode/default strict-mode, `tokenStatus=needs_refresh` through CLI recovery, and bounded OpenCLI daemon/doctor startup. Do not add fallback business code or bypass external dependencies to hide missing auth/runtime.
  4. If the blocker needs human action, stop the workflow and give exact steps. Human blockers include Facebook logged out, visitor preview, CAPTCHA/risk control, wrong Chrome profile, Browser Bridge extension not connected to the business Chrome profile after bounded recovery, or target page posts not visibly loaded.
  5. Business user keeps Facebook logged in through the normal Chrome profile; batch jobs should open the target account homepage through OpenCLI automation unless `--no-open-account-tabs` is explicit.
  6. OpenCLI Browser Bridge lists tabs and uses an explicit automation `--tab-page` or a matching Facebook account tab; it must not unconditionally `browser bind` or occupy the user's current active tab. Direct `--tab` reads are preferred, and select fallback is only allowed for tabs opened and tracked by automation.
  7. The intended operator experience is no interruption to other software or other Chrome windows. Current code supports page-id/tab isolation by opening account pages with OpenCLI `tab new` and passing `--tab-page`; if strict separate Chrome-window isolation is required later, first add/verify explicit runtime support instead of claiming it in docs only.
  8. Homepage relative labels such as `3h`, `12h`, `1d` only define the candidate window and scroll boundary.
  9. Candidate detail enrichment reuses one detail tab when possible, with the original fresh-tab flow as fallback, to confirm exact `posted_at`, expand comments/replies, capture the account-owned lead link, read engagement, classify `post_type`, and collect article material.
  10. The lead link resolves to an external `landing_url`, article material is fetched from the landing page, and the Chinese `story_summary` must be based on that material.
  11. Incomplete rows are stored in SQLite and补抓 queues first. Missing exact time, lead link, engagement, post type, article material, or story summary must trigger scoped resume/enrichment/summary generation until no machine-runnable work remains or a hard blocker appears.
  12. Normal business `--sync` is strict final output: only current `ready_for_output` rows that pass `quality_audit` are written to the formal Feishu table. Incomplete candidates remain in SQLite, enrichment queues, and Codex summaries, not as `待补抓` rows in the formal table.
  13. Use `--sync-audit` or `--ledger-sync` only when the operator explicitly wants the legacy auditable ledger/preview behavior that writes incomplete rows with `待补抓：...` markers. `--strict-ready-only` is retained only as a compatibility no-op for old commands.
  14. Business “抓取并写入飞书” runs should use `scripts/run_account_job.py`, which persists progress in SQLite, resumes scoped enrichment after interruptions, and reports `run_status`. A ledger write with pending enrichment is not a completed job.
  15. Business “抓取全部/所有目标账号并写入飞书” runs should use `scripts/run_accounts_job.py`, which reads the Feishu account source sheet and runs the full `run_account_job.py` flow for each enabled account. Do not hand-stitch per-account commands in chat and then forget later accounts or pending detail fields.
  16. If multiple accounts are in scope, run the full single-account capture/enrichment/strict-sync loop for one account, close that automation-opened homepage tab, then move to the next account. Continue later accounts on recoverable per-account failures, but preserve top-level `next_commands` for the original batch request.
  17. If homepage discovery or detail enrichment sees `login_required`, `visitor_preview`, `facebook_tab_missing`, or `human_intervention_required`, the account job must stop with `run_status=human_intervention_required`; do not import/sync visitor-preview rows or hide this as a generic failed enrichment task.
  18. Final Codex output for a capture job must summarize accounts attempted, candidate posts found, strict final synced/usable rows, whether all required fields are complete, remaining blockers/next commands, and any extreme special cases such as a post with almost no fields captured.
...
- For direct post/photo/video/reel/watch/share URLs, treat the request as a narrow scoped capture/enrichment job. If the post is already in SQLite, resume补抓 for its account/date scope without overwriting stronger existing fields. If it is not in SQLite, import it as a candidate with account context when available, then run the normal exact-time, lead-link, engagement, post-type, article-material, summary, and strict-sync path. If account URL/date cannot be inferred, ask for the account page URL or visible account name before live capture.
...
- The formal Feishu table is now strict final output by default. If a Facebook post candidate is confirmed by URL and account context but still misses exact time, lead link, article summary, engagement, post type, or capture coverage, normal business `--sync` must keep it local and report the gap instead of writing a `待补抓` row. Explicit `--sync-audit` / `--ledger-sync` can still upsert those auditable ledger rows when the operator asks for preview/ledger mode.
...
- `run_accounts_job.py` is the batch orchestration entrypoint for “all target accounts.” It must continue iterating through the configured account list even when one account returns an incomplete local/enrichment state, and it must aggregate each account's `run_status`, `completion_blockers`, and first `next_commands`. By default it opens each target account homepage through OpenCLI, passes that page id into the child account job with `--tab-page`, runs each account job with strict completion, automatically follows same-account machine-runnable `next_commands` for at least the base `--auto-follow-attempts` budget and keeps going while quality metrics improve, and closes that automation-opened homepage tab immediately after the account finishes. Do not wait until the end of the whole batch to close account homepage tabs; doing so can accumulate many Facebook tabs and freeze Chrome. `--no-open-account-tabs` is only for cases where the operator intentionally pre-opened matching account tabs. Hard blockers such as Feishu auth, OpenCLI not connected, login/profile issues, or account-tab mismatch remain explicit per-account blockers; do not silently switch to another Facebook tab.
...
- `run_account_job.py` supports explicit quality threshold gates for acceptance/automation: `--require-coverage-complete`, `--min-ledger-usable-rate`, `--min-final-usable-rate`, `--min-completion-rate`, `--min-expected-post-coverage-rate`, and `--min-expected-label-coverage-rate`. Defaults are zero/off because normal business `--sync` already uses the strict final-output gate; when thresholds are supplied, failures emit `quality_threshold_failed`, `exit_status_reason=quality_threshold_failed`, and recovery hints without hiding the underlying `run_status`.
...
- `scripts/import_existing_result.py`: import JSON/CSV into SQLite and optionally strict-sync complete rows with `--sync`; it reports `enrichment_tasks.stage_counts` / `open_stage_counts` so callers can see which补抓 stages block final usable rows. Use `--sync-audit` / `--ledger-sync` only for explicit incomplete ledger output.
- `scripts/filter_posts.py`: filter local SQLite records and strict-sync complete rows with `--sync`; use `--sync-audit` / `--ledger-sync` only for explicit incomplete ledger output.
- `scripts/sync_feishu.py`: sync local records to Feishu using strict ready-row output by default; use `--audit`, `--sync-audit`, or `--ledger-sync` only for explicit incomplete ledger output. When called with `conn`, it first refreshes the scoped SQLite rows with current config audit/status and queues missing business-field enrichment tasks, so historical rows with a post link but empty `帖子类型` or `故事概要` cannot remain silent. Direct sync results from import/filter/sync paths include `completion_blockers`; `run_status=incomplete_pending_tasks` means no incomplete formal rows were written and local rows still need补抓.
...
Strict-sync complete rows to the formal table:

```bash
python3 scripts/import_existing_result.py --config config/settings.yaml --input exports/prepared.json --sync --dry-run
```

Explicit legacy audit/ledger sync for incomplete rows, only when explicitly requested:
