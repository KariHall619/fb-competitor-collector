...
- “采集全部目标账号/所有账号今天的帖子并写入飞书”
  ...
  - It reads the Feishu account source sheet, calls the full `run_account_job.py` flow for each enabled account, automatically follows same-account machine-runnable recovery commands for the base `--auto-follow-attempts` budget (default 8), extends while field-completion quality or stage pressure is still improving, and aggregates per-account `run_status`, `completion_blockers`, and `next_commands`.
...
- If a single-account result includes both coverage and field-completion commands, use the first scoped `pending_enrichment` resume command before another homepage coverage rerun. This prevents already imported posts from staying in Feishu without detail fields while coverage is still being improved.
