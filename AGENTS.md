...
- `scripts/run_accounts_job.py`: preferred batch business entrypoint for all configured accounts. It reads the Feishu account source sheet, calls `run_account_job.py` for each enabled target account, automatically follows same-account machine-runnable recovery commands up to `--auto-follow-attempts`, aggregates account-level status, and returns nonzero by default if any account is incomplete or blocked.
