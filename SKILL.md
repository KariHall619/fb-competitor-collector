...
- `run_capture_pipeline.py` also reports `quality_summary`, `feishu_sync`, and `enrichment_tasks`; if it returns `complete=false`, treat the result as imported/queued work, not as final collection completion. Use `--fail-on-incomplete` when a caller must not accept incomplete partial imports.
- Stable no-new-post extraction stop is normal completion. Business account capture defaults to `--max-snapshots 32 --min-snapshots 6` so Facebook virtualized feeds get multiple scroll samples before stopping. If current-tab extraction returns `coverage_incomplete=true`, the run hit the snapshot cap while still finding new candidates. Raise `--max-snapshots` or restart from the account homepage top before declaring the visible date window fully covered.
